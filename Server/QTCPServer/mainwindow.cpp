...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    if(myServer->listen(QHostAddress::Any, 8080))
    {
       ...
       connect(myServer, &QTcpServer::newConnection, this, &MainWindow::newConnection);
       ui->statusBar->showMessage("Server is listening...");
    }
    else
    {
        QMessageBox::critical(this,"QTCPServer",QString("Unable to start the server: %1.").arg(myServer->errorString()));
        exit(EXIT_FAILURE);
    }
}
...
void MainWindow::newConnection()
{
    while (myServer->hasPendingConnections())
        appendToSocketList(myServer->nextPendingConnection());
}
